...
class Auth extends GS_Controller
{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						...
						$this->form_validation->set_value('password'),
						$email_activation,
						$this->form_validation->set_value('sex')))) {									// success
	...
	function finish_preregistering()
	{
	    if ($this->tank_auth->is_logged_in()) {									// logged in
			redirect('');
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->finish_registering_user(
						$this->form_validation->set_value('email'),
		...
		$data['title'] = 'Finish preregistering';
	    $data['content_view'] = 'auth/finish_preregistering';
	    $this->load->view('skeleton', array_merge($data, $this->data));
